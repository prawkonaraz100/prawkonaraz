...
const props = withDefaults(defineProps<{
    ...
    showPosterWhilePreparing?: boolean;
...
const updateContentRect = () => {
    ...
    if (!container || !video || !naturalWidth || !naturalHeight) {
        return;
    }
...
const resolveFrameTime = (video: HTMLVideoElement) => {
    if (props.frameTimeSeconds === null || Number.isNaN(video.duration) || video.duration <= 0) {
        return 0;
    }

    return Math.min(Math.max(props.frameTimeSeconds, 0), Math.max(video.duration - 0.05, 0));
};

const prepareFrame = () => {
    const video = videoRef.value;

    if (!video || !metadataReady.value) {
        return;
    }

    const targetTime = resolveFrameTime(video);
    ...
    try {
        ...
        // Browser may reject early seeks while metadata finalizes.
    }
};
...
const handleLoadedMetadata = () => {
    ...
    measuredNaturalWidth.value = video.videoWidth || props.naturalWidth || video.clientWidth || null;
    measuredNaturalHeight.value = video.videoHeight || props.naturalHeight || video.clientHeight || null;
...
<template>
    ...
    >
        <img
            v-if="showPosterWhilePreparing && !frameReady && poster"
            :src="poster"
            alt=""
            aria-hidden="true"
            class="pointer-events-none absolute inset-0 h-full w-full object-contain bg-white"
        >
        <video
            ref="videoRef"
            :src="src"
            ...
            class="pointer-events-none transition-opacity duration-150 ease-out"
            :class="[videoClass, frameReady ? 'opacity-100' : 'opacity-0']"
        ...
        <div
            v-if="frameReady && renderableAnnotations.length > 0 && contentRect.width > 0 && contentRect.height > 0"
