...
class TrafficSignHorizontalInventorySeeder extends Seeder
{
    private const BATCH_LABEL = 'rollout-15-horizontal-inventory';
    ...
    public function run(
        ...
        foreach ($polishHorizontalSignCatalog->all() as $index => $signData) {
            ...
            if (! $sign->exists) {
                $content = $polishHorizontalSignContentBuilder->build($signData, $index);
            ...
            TrafficSignQueryMapEntry::query()->updateOrCreate(
                ...
                [
                    ...
                    'traffic_sign_category_id' => $category->getKey(),
                    'watch_reason' => 'Element inwentarza znaków P (poziome).',
                    'source_plan' => 'Oprzeć stronę o krytyczne zasady jazdy i zmiany pasów ruchu.',
                    'correction_notes' => 'Upewnić się, że różnice między najechaniem a przekroczeniem są ujęte.',
                    'competitor_notes' => 'Wpis buduje kompletność katalogu znaków poziomych.',
                    'first_mover_note' => null,
                    'notes' => 'Wpis dodany automatycznie z inwentarza znaków poziomych.',
                ],
            );
        }
    }
