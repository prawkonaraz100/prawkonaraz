...
class TrafficSignComplementaryInventorySeeder extends Seeder
{
    private const BATCH_LABEL = 'rollout-12-complementary-inventory';
    ...
    public function run(
        ...
        foreach ($polishComplementarySignCatalog->all() as $index => $signData) {
            ...
            if (! $sign->exists) {
                $content = $polishComplementarySignContentBuilder->build($signData, $index);
            ...
            TrafficSignQueryMapEntry::query()->updateOrCreate(
                ...
                [
                    ...
                    'traffic_sign_category_id' => $category->getKey(),
                    'watch_reason' => 'Element inwentarza polskich znaków uzupełniających. Chcemy utrzymać pokrycie kategorii.',
                    'source_plan' => 'Oprzeć stronę o organizację ruchu przypisaną poszczególnym pasom ruchu.',
                    'correction_notes' => 'Przed publikacją doprecyzować sytuacje na skrzyżowaniach.',
                    'competitor_notes' => 'Wpis utrzymuje kompletność katalogu znaków F.',
                    'first_mover_note' => null,
                    'notes' => 'Wpis dodany automatycznie z kanonicznego inwentarza znaków uzupełniających.',
                ],
            );
        }
    }
