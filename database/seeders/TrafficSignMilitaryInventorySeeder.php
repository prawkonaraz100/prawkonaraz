...
use App\Support\PolishMilitarySignCatalog;
use App\Support\PolishMilitarySignContentBuilder;
use App\Support\TrafficSignAuthorProfile;
use Illuminate\Database\Seeder;

class TrafficSignMilitaryInventorySeeder extends Seeder
{
    private const BATCH_LABEL = 'rollout-19-military-inventory';

    public function run(
        PolishMilitarySignCatalog $polishMilitarySignCatalog,
        PolishMilitarySignContentBuilder $polishMilitarySignContentBuilder,
    ): void {
        ...
        foreach ($polishMilitarySignCatalog->all() as $index => $signData) {
            $sign = TrafficSign::query()->firstOrNew([
                'code' => $signData['code'],
            ]);

            if (! $sign->exists) {
                $content = $polishMilitarySignContentBuilder->build($signData, $index);
            ...
            TrafficSignQueryMapEntry::query()->updateOrCreate(
                ...
                [
                    ...
                    'traffic_sign_category_id' => $category->getKey(),
                    'watch_reason' => 'Znak wojskowy w inwentarzu.',
                    'source_plan' => 'Wyjaśnić pojęcie MLC kierowcom.',
                    'correction_notes' => 'Zaznaczyć, że cywilów on nie dotyczy.',
                    'competitor_notes' => 'Pojawia się w niszowych pytaniach egzaminacyjnych.',
