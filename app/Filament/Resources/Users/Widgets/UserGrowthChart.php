...
class UserGrowthChart extends ChartWidget
{
    ...
    {
        return sprintf(
            'Rejestracje w czasie i skala całej bazy kont. Łącznie %s użytkowników, %s zweryfikowanych, %s administratorów i %s zablokowanych.',
            number_format(User::query()->count(), 0, ',', ' '),
            number_format(User::query()->whereNotNull('email_verified_at')->count(), 0, ',', ' '),
            number_format(User::query()->where('is_admin', true)->count(), 0, ',', ' '),
            number_format(User::query()->whereNotNull('banned_at')->count(), 0, ',', ' '),
        );
    }
    ...
    protected function getData(): array
    {
        ...
        User::query()
            ...
            ->orderBy('created_at')
            ->get(['created_at'])
            ->each(function (User $user) use (&$counts, $mode): void {
        ...
        foreach ($buckets as $key => $bucket) {
            $bucketCount = $counts[$key] ?? 0;
            $running += $bucketCount;

            $labels[] = $bucket['label'];
            $newUsers[] = $bucketCount;
            $cumulative[] = $running;
        }
