...
Zakres: niepubliczne porównanie V1 z zatwierdzonym runem V2 dla jednego klastra
...
Na produkcji aktywny jest tylko odczyt wewnętrzny dla `secondary:zawracanie`.
Run V2 nr 2 ma status `validated`; rekord rollout nr 1 ma `mode=shadow`,
aktywny run nr 2 i ekspozycję `0`. Żadna odpowiedź HTML, użytkownik ani robot
wyszukiwarki nie otrzymuje V2.
...
Resolver preview może odczytać istniejący `mode=shadow` przed włączeniem flag
ruchu.
...
udostępnia V2 dla jednego pytania; zwykły URL nadal renderuje V1. Preview ma
`noindex,nofollow,noarchive`, nie emituje JSON-LD i nie jest canary. Pełny
runbook: `docs/SEO-RELATION-V2-SIGNED-PREVIEW.md`.
...
Canary oraz `mode=v2` są poza tym runbookiem i wymagają osobnej decyzji.
Przygotowanie technicznej ścieżki canary opisuje
`docs/SEO-RELATION-V2-CANARY-RUNBOOK.md`; nie oznacza ono wdrożenia ani
włączenia ruchu publicznego.
