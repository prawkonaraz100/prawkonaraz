...
4. rozpakowac paczke,
5. ustawic wlasciciela dla `public/build` i nowe katalogi assetow oraz zachowac
   grupowy zapis `www-data` do `public/` i `public/sitemaps/` dla atomowego
   odswiezania sitemap przez scheduler,
...
install -d -o root -g www-data -m 2775 "${APP_DIR}/public/sitemaps"
chgrp www-data "${APP_DIR}/public"
chmod 2775 "${APP_DIR}/public"
...
```bash
runuser -u www-data -- php artisan seo:refresh-sitemaps
runuser -u www-data -- php artisan seo:audit-sitemaps
```

Komendy sitemap uruchamiamy jako ten sam uzytkownik co cron. Generator tworzy
pliki `*.tmp.*` obok docelowych XML-i i wykonuje atomowa podmiane, dlatego
`www-data` musi miec zapis do obu katalogow. Sam zapis do istniejacego
`sitemap.xml` nie wystarcza. Po deployu sprawdz:

```bash
runuser -u www-data -- test -w public
runuser -u www-data -- test -w public/sitemaps
find public -maxdepth 2 -type f -name '*.tmp.*'
```

Ostatnia komenda nie powinna zwrocic zadnych plikow.
