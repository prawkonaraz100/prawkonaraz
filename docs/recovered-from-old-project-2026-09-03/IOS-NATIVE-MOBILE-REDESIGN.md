...
## W przygotowaniu 2026-07-13 - visual redesign `/nauka`

- redesign jest prowadzony na branchu `codex/mobile-learning-visual-redesign` i nie zmienia zatwierdzonego flow: bezposredniej rekomendacji, pelnoekranowego konfiguratora, sciezki dzialow ani egzaminu;
- caly mobilny ekran glowny ma nowa hierarchie aplikacyjna: osobny tytul destination `Nauka`, skrot postepu, glowny plan, modul egzaminu oraz modul kursu. Nie jest to zmiana ograniczona do hero ani zestaw niezaleznych kafli;
- plan na teraz pokazuje rzeczywisty temat, liczbe pytan, kategorie oraz pojazd dobrany do A/AM/B/C/C1/D/T zamiast jednego samochodu osobowego dla wszystkich kursantow;
- `Egzamin probny` jest osobnym, jednoznacznym zadaniem z liczba pytan, czasem i informacja o formacie egzaminu panstwowego; zachowuje ten sam pelnoekranowy krok potwierdzenia;
- `Twoj kurs` laczy pierscien opanowania, liczbe przerobionych i pozostalych pytan oraz zwarte wyniki poprawne/bledne. Prowadzi do istniejacego widoku szczegolowego postepu i nie przenosi rekordow na ekran glowny;
- paleta rozdziela role: granat buduje kontekst nauki i pojazdu, pomaranczowy oznacza glowna decyzje, niebieski pozostaje kolorem postepu i zaznaczen pomocniczych, a zielen/czerwien opisuja wyniki;
- app bar zachowuje wybrana kategorie, typ pojazdu, placeholder ustawien, imie i avatar. Konfigurator ma trzy numerowane etapy, a sciezka dzialow naprzemienny przebieg przypominajacy trase; oba widoki korzystaja z pelnoekranowego shellu i stalej akcji bez zmiany ich logiki;
- quality gate obejmuje build produkcyjny, brak poziomego overflow, czysta konsole oraz kontrolowane widoki `360x800` i `390x844`; player `/nauka/teraz` pozostaje poza zakresem.
