...
## Aktualizacja 2026-07-13 - swiadome stany puste

Dodano wspolny, bezkartowy `MobileEmptyState.vue`, aby brak danych nie wygladal jak niedoladowana albo uszkodzona sekcja. Komponent rozroznia neutralny brak historii od pozytywnego braku problemow, zachowuje zwarta wysokosc i moze korzystac z ikon aktywnosci, sukcesu, przekroju albo pustej kolejki.

Wdrozone zastosowania:

- brak aktywnosci w ostatnich siedmiu dniach;
- brak pytan do poprawy przed pierwsza seria oraz pozytywny brak pilnych pytan po rozpoczeciu nauki;
- za malo danych dla wybranego przekroju analityki;
- egzamin `100%` bez pytan wymagajacych ponownego przejrzenia;
- zakonczony plan `/trener-pamieci`: zamiast neutralnego przycisku i malej adnotacji kursant widzi pozytywny stan, informacje o kolejnym terminie oraz jedno wyjscie do nauki;
- pusty korpus `/nauka/znaki-drogowe`: zamiast nieaktywnego glownego CTA widoczny jest neutralny komunikat, a niedzialajacy wybor trybu i pusta lista kategorii nie sa renderowane.

Zweryfikowano realna kategorie bez zapisanej historii oraz lokalna zakonczona sesje egzaminacyjna `32/32` na `360x800`, a takze brak overflow na `390x844`, `430x932` i zachowany desktop `1440x900`. Puste warianty trenera i znakow wyrenderowano przez kontrolowana odpowiedz Inertia w przegladarce QA, bez zapisu w bazie; oba zachowuja dock, nie maja martwego CTA ani horizontal overflow na `360x800` i `430x932`. Konsola nie pokazala bledow. Regresje: 2 testy strony analityki, 4 testy API analityki, 2 testy flow wyniku egzaminu oraz 30 testow trenera pamieci i znakow z 432 asercjami. Player nie byl modyfikowany.
