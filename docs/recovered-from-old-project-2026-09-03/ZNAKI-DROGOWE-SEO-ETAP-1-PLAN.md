...
- publiczne schema znakow zostalo podniesione do enterprise graph dla huba, kategorii i strony znaku: jeden JSON-LD `@graph`, `DefinedTermSet` dla bazy znakow, `DefinedTerm` dla kategorii i znaku, `ItemList` dla list, `Article`/`FAQPage`/`ImageObject` na stronie znaku oraz guardrail testowy dla `/nauka`.

### Nastepny konkret

- szczegolowy status refaktoru schema znakow jest w [docs/TRAFFIC-SIGN-ENTERPRISE-SCHEMA-PLAN.md](C:/Users/xxx/Desktop/serwistestyprawojazdy/docs/TRAFFIC-SIGN-ENTERPRISE-SCHEMA-PLAN.md),
