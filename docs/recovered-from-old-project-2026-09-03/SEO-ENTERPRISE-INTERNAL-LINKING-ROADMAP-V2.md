...
- `docs/SEO-RELATION-V2-CANARY-RUNBOOK.md` — aktywny publiczny rollout V2 dla całego pilota oraz procedura monitoringu i rollbacku,
...
- istnieje walidowany snapshot V2, chroniony preview administratora i cykliczny monitoring; runtime shadow jest wyłączony po przejściu do canary, a przed nami pozostają dedykowana analityka kliknięć oraz redakcyjny workflow różnic,
- publiczny canary V2 działa dla całego pilota (29/29 źródeł); każdy kolejny topic wymaga nowej zgody, preview, backupu i odrębnej ścieżki zmiany,
- proste, szablonowe treści hubów,
- pilot „Zawracanie” ma skorygowany run V2 nr 2, zakończony P0 self-review i aktywny rollout 100%; pełny audyt redakcyjny, subhuby i skalowanie na kolejne topici pozostają do wykonania.
...
| feature flagi per klaster i algorytm | `PARTIAL` | canary V2 działa dla całego pilota na 100% (29/29 źródeł); każdy kolejny topic wymaga osobnej decyzji |
| chroniony preview i monitoring shadow | `DONE` | panel Filament admina, komenda quality gate i historyczny harmonogram 04:00; runtime shadow jest obecnie wyłączony |
| canary konfigurator i monitoring | `PARTIAL` | konfigurator, dedykowana promocja pełnego topicu i monitor są wdrożone; output publiczny 100% pilota jest aktywny, brak dashboardu i alertów |
...
Status: `PRODUCTION V2 FULL-TOPIC ROLLOUT + MONITORING ACTIVE — V1 W INNYCH TOPICACH`
Szacunek: 7–15 dni
...
Status: `PARTIAL — historyczny shadow, preview i aktywny pełny rollout pilota; pozostałe elementy TODO`
...
Status: `FULL-TOPIC CANARY ACTIVE / MONITOR 0 ERRORS, 0 WARNINGS`
Szacunek obserwacji: minimum 2–4 tygodnie

Zakres:

- [x] włączyć V2 dla całego klastra „Zawracanie” (100%, 29/29 źródeł, po 15 linków),
...
- [x] `SEO-LINK-V2-007` — wdrożono shadow jako etap historyczny, następnie aktywowano `canary 5%`, a po osobnym preview i backupach promowano rollout nr 1 do `canary 100%` runu nr 2 dla wszystkich 29 źródeł; monitor ma 0 błędów,
- [x] `SEO-LINK-V2-008` — monitoring jakości shadow: komenda quality gate, raport JSON, czas audytu oraz harmonogram produkcyjny; runtime shadow wyłączono po aktywacji canary,
...
- [x] `SEO-LINK-V2-010` — generator shadow i idempotentny adapter utworzyły na produkcji skorygowany run `validated` nr 2: 29 źródeł core i 435 rekomendacji (181 `direct`, 254 `context`); rollout nr 1 wskazuje ten run jako `canary` 100%,
...
- [ ] `SEO-LINK-V2-019` — `PARTIAL`: konfigurator, bezpieczna promocja pełnego topicu i monitor canary działają publicznie dla 29/29 źródeł „Zawracania”; pozostają dashboard, alerty i decyzja o rolloutcie kolejnego topicu.
...
Cel sprintu: bezpiecznie obserwować aktywny rollout V2 dla całego pilota „Zawracanie”, zachowując V1 w innych topicach i jako rollback.
...
8. ~~`SEO-LINK-V2-010` / `011` — niepubliczny snapshot shadow dla 29 źródeł core i 435 rekomendacji.~~ `DONE`: produkcyjny, skorygowany run nr 2 ma status `validated`; po shadow rollout przeszedł do `canary 100%` dla wszystkich 29 źródeł,
9. ~~`SEO-LINK-V2-007` — wdrożenie i aktywacja feature flag shadow z ekspozycją 0%, następnie audyt V1 ↔ V2.~~ `DONE`: etap shadow miał 0 błędów, po czym rollout nr 1 przeszedł przez `canary 5%` do `canary 100%`; V1 pozostaje w innych topicach,
...
13. `SEO-LINK-V2-018` — zdarzenia analityczne dla aktywnego publicznego rolloutu V2.
14. `SEO-LINK-V2-019` — backup, preview, jawny zapis canary, promocja pełnego topicu i monitoring wykonano; pozostają dashboard, alerty i decyzja o rolloutcie kolejnego topicu. Runbook: `docs/SEO-RELATION-V2-CANARY-RUNBOOK.md`.
...
| 2026-07-28 | wdrożono domyślnie wyłączony mechanizm canary: dwa wyłączniki, stabilną kohortę, jawny konfigurator i monitor; `canary_enabled=false`, rollout pozostał `shadow` 0%, health/smoke i zwykły SSR V1 są OK |
| 2026-07-28 | production preview canary 5% dla runu nr 2 przeszedł wszystkie quality gates; seed `zawracanie-v2-canary-2026-07-28-1` wskazuje 2/29 źródeł (1178, 6019), ale zapis i publiczna ekspozycja nie zostały uruchomione |
| 2026-07-28 | po odrębnej zgodzie utworzono backup bazy oraz runtime/configu, włączono flagę canary i atomowo przełączono rollout nr 1 na `canary 5%` runu nr 2; V2 renderuje pytania 1178 i 6019, V1 pozostałe 27 źródeł, a pierwszy monitor, health/smoke i kontrola HTTP/canonicali są poprawne |
| 2026-07-28 | po wdrożeniu chronionej ścieżki promocji, nowym preview i świeżych backupach atomowo podniesiono rollout nr 1 z `canary 5%` do `canary 100%`; V2 renderuje wszystkie 29/29 źródeł „Zawracania” po 15 linków, V1 pozostaje w innych topicach, a monitor, health/smoke i kontrola HTTP/canonicali są poprawne |
| 2026-07-28 | naprawiono listę pytań głównego hubu „Zawracanie”: członkostwa V2 są rozwiązywane po identyfikatorze publicznego pytania, więc hub renderuje 29 kanonicznych linków na dwóch stronach zamiast błędnego komunikatu „0 pytań” |
