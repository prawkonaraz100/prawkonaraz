...
Status: `P0 COMPLETE / P1 CONTENT WORK REMAINS / PUBLIC V2 FULL-TOPIC ACTIVE`

Data audytu: 2026-07-28

Zakres: produkcyjny topic `secondary:zawracanie`, run V2 nr `2`, rollout nr `1` w trybie `canary` z ekspozycją `100%`.

## Decyzja po audycie

Po osobnej decyzji właściciela, preview i backupach promowano publiczny canary
V2 do `100%`: wszystkie 29 źródeł topicu renderuje V2, natomiast inne topici
nadal renderują V1. P0 pozostaje zamknięte: dwa znane błędne linki `direct`
wykluczono w wersjonowanym artefakcie i aktywnym runie nr 2, a reprezentatywne
pytania wraz z medium oraz podstawami prawnymi przeszły self-review. Pozostaje
praca P1: pełne dossier treści, zakresy subhubów i redakcyjny przegląd różnic.

Ten dokument nie tworzy nowych publicznych URL-i; decyzję o aktywacji i jej
backupy dokumentuje `docs/SEO-RELATION-V2-CANARY-RUNBOOK.md`.
...
5. Przy każdej kolejnej korekcie wygenerować nowy artefakt i walidowany run,
   wykonać audyt V1 ↔ V2 oraz monitor. Aktywny canary nie może zostać
   zmieniony automatycznie: jego zastąpienie, zwiększenie ekspozycji albo
   rollback wymagają osobnej decyzji i procedury z runbooka.
...
- [x] V2 renderuje wszystkie 29/29 źródeł `secondary:zawracanie` przy
  ekspozycji 100%; V1 pozostaje rendererem publicznym w innych topicach oraz
  rollbackiem flagą canary.
