...
- `sign_up` is sent after a successful account registration and only includes the registration method.
- `begin_checkout` is sent when a pending checkout is opened.
- `purchase` is sent once per browser session after a paid order is shown, with the order reference and ecommerce details required by GA4.
- Event dispatch respects the same consent choice as the Google tag. No email address, name, or other identifying customer data is sent with these events.
