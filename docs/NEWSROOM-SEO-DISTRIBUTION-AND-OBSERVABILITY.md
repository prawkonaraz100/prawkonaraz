...
- PR #129 rozszerzył ten sam N6-003 validator o production-only canonical-host migration checks dla apex HTTP, obu wariantów `www` i historycznego `https://prawkoapp.pl/`; wszystkie mają redirectować do `https://prawkonaraz.pl/`, a custom/non-production `BASE_URL` jawnie pomija project-specific host guard,
- najnowszy report-only N6-003 run #11 z 2026-09-20 nadal potwierdza production drift mimo workflow conclusion `success`: underlying static-delivery smoke exit `1`, enterprise validator exit `1` (`102 checks / 3 failures / 2 warnings`). Live `/robots.txt` ma `max-age=14400`; homepage WebSite/Organization nodes nie mają stable `/#website` i `/#organization` @id; `/aktualnosci` nie ma repo placeholder `noindex`; root sitemap nadal nie ma article/news coverage, feed pozostaje 404. Current `main` ma już wszystkie trzy kontrakty i regression tests, więc nie zmieniamy aplikacji pod stary runtime,
- GSC evidence 2026-09-20 jest częściowe, ale poprawiło się dla homepage: `sitemap.xml` jest submitted i ostatnio pobrany 2026-09-19 bez reported warnings/errors; canonical HTTPS homepage ma teraz URL Inspection `PASS / Submitted and indexed`, robots/indexing allowed, successful fetch i last crawl 2026-09-19 17:02:38Z. Wcześniejszy `Duplicate, Google chose different canonical than user` pozostaje historycznym verdict. `/aktualnosci` nadal jest `URL is unknown to Google`, a settled Search Analytics do 2026-09-17 ma dla tego URL 0 clicks / 0 impressions. Referring URLs homepage nadal obejmują `https://prawkoapp.pl/` i `http://prawkonaraz.pl/`, ale przy aktualnym PASS nie stanowią samodzielnego dowodu canonical failure. Brak representative live article/category/topic oznacza, że N6-003 i pełna production Search Console verification pozostają IN PROGRESS,
...
- [x] dodać N6-003 canonical-host migration validation dla apex HTTP / `www` / legacy `prawkoapp.pl` i zapisać GSC evidence bez przypisywania redirect PASS jako rozwiązania Google-selected canonical mismatchu — PR #129,
...
- [ ] wykonać pełną production Search Console verification — homepage ma już 2026-09-20 URL Inspection PASS / Submitted and indexed, ale `/aktualnosci` pozostaje unknown i brak representative live article/category/topic evidence,
...
- odświeżono N5-007/N6-003 live evidence bez zmian implementacji: current repo contract i testy już wymagają stable site-identity `@id`, placeholder `noindex` i crawler-safe Nginx cache,
- Enterprise SEO Production Validation #11 jest REPORT-ONLY wrapper success, ale static smoke i enterprise validator mają underlying exit `1`; enterprise raportuje `102 checks / 3 failures / 2 warnings`,
- GSC URL Inspection dla canonical homepage ma teraz `PASS / Submitted and indexed` po crawl 2026-09-19 17:02:38Z, podczas gdy `/aktualnosci` pozostaje unknown to Google; settled Search Analytics do 2026-09-17 ma dla newsroom hub 0 clicks / 0 impressions,
- N5-007/N6-003 pozostają IN PROGRESS do aktualnego live deploy/runtime, zielonego STRICT evidence i representative newsroom samples.

### 2026-09-19 — v0.32

- PR #129 rozszerzył istniejący N6-003 production validator o canonical-host migration checks bez zmiany aplikacji, Nginx, rollout gate ani SEO architecture,
- report-only Enterprise SEO Production Validation #3 potwierdził apex HTTP, `https://www` i legacy `https://prawkoapp.pl/` jako 1-hop redirect do canonical HTTPS homepage; `http://www` również kończy poprawnie, ale przez 2 hops (warning),
- GSC URL Inspection 2026-09-19 nadal pokazuje `Duplicate, Google chose different canonical than user` dla canonical HTTPS homepage; apex HTTP jest `Page with redirect`, oba `www` są unknown, a `prawkoapp.pl` pozostaje referring URL, więc aktywna host migration nie jest potwierdzoną przyczyną mismatchu,
...
- N6-003/N5-007 nadal pozostają otwarte do faktycznego production deploy/runtime, zielonego STRICT evidence, live stable `@id`/placeholder contract oraz representative article/category/topic + GSC validation.
