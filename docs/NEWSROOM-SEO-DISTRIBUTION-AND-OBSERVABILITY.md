...
- GSC evidence 2026-09-19 jest częściowe, nie finalne: `sitemap.xml` jest submitted i pobrany tego dnia bez reported warnings/errors; homepage URL Inspection zwraca `Duplicate, Google chose different canonical than user`, a `/aktualnosci` `URL is unknown to Google`; dodatkowe inspection pokazuje `http://prawkonaraz.pl/` jako `Page with redirect`, oba `www` jako unknown to Google oraz `https://prawkoapp.pl/` jako referring URL homepage. Enterprise SEO Production Validation #3 potwierdza HTTP redirect-to-canonical PASS dla wszystkich tych host variants; `http://www` ma 2-hop warning, pozostałe sprawdzone warianty 1 hop. Host migration nie jest więc potwierdzoną aktywną przyczyną Google-selected canonical mismatchu; brak representative live article/category/topic oznacza, że N6-003 i production Search Console verification pozostają IN PROGRESS,
